<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h1 class="font-extrabold text-2xl text-slate-800 tracking-tight">تعديل الدور والصلاحيات: <span class="text-blue-600">{{ $role->name }}</span></h1>
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('roles.update', $role->id) }}" class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-2">اسم الدور *</label>
                    <input type="text" name="name" value="{{ old('name', $role->name) }}" required class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-3">تحديد وتخصيص الصلاحيات الممنوحة له الموقف:</label>
