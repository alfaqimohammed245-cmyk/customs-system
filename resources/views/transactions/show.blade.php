<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
                @can('طباعة')
                <button onclick="window.print()" class="bg-slate-800 hover:bg-slate-900 text-white font-bold px-4 py-2 rounded-xl text-sm transition flex items-center gap-1.5">
    ...
    <div class="py-8 print:py-0 print:bg-white">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6">
                ...
                <div class="w-full md:w-80">
                    ...
                    <div class="w-full bg-slate-100 rounded-full h-3 overflow-hidden p-0.5 border border-slate-200/60">
                        <div class="bg-gradient-to-r from-blue-600 to-indigo-600 h-2 rounded-full transition-all duration-500" style="width: {{ $transaction->progress_percentage }}%"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-6">
                <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                    ...
                    @can('تعديل')
                    <a href="{{ route('transactions.edit', $transaction->id) }}" class="text-xs font-bold text-blue-600 hover:underline">
                        تعديل بيانات المراحل &larr;
                    </a>
                    @endcan
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
                    ...
                    <div class="rounded-xl p-3 border transition relative flex flex-col justify-between
                        ...
                        @elseif($isCurrent) bg-blue-50 border-blue-400 ring-2 ring-blue-200
                        @else bg-slate-50 border-slate-200 opacity-80 @endif">
                        ...
                        <div>
                            ...
                            <div class="flex items-center justify-between mb-2">
                                <span class="w-6 h-6 rounded-lg text-xs font-black flex items-center justify-center
                                    ...
                                    @elseif($isCurrent) bg-blue-600 text-white animate-pulse
                                ...
                                @elseif($isCurrent)
                                <span class="text-blue-700 text-[10px] font-bold flex items-center gap-1">
                                    <i class="fa-solid fa-spinner fa-spin"></i> جارية الآن
                                </span>
                                @else
                        ...
                        <div class="mt-3 pt-2 border-t border-slate-200/60">
                            ...
                            @else
                            <span class="block text-center text-[9px] text-slate-400 font-medium">🔒 صلاحية المرحلة مقفلة</span>
                            @endif
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
                    <h3 class="font-bold text-slate-800 border-b border-slate-100 pb-3 flex items-center justify-between">
                        <span class="flex items-center gap-2">
                            <i class="fa-solid fa-clipboard-list text-blue-600"></i> 1. بيانات الاستلام والترقيم
                        </span>
                    ...
                    <div class="grid grid-cols-2 gap-4 text-xs">
                        ...
                        @if($transaction->stage_1_notes)
                        <div class="col-span-2">
                            <span class="text-slate-400 block mb-1">ملاحظات مرحلة الاستلام:</span>
                            <p class="text-slate-700 bg-slate-50 p-2.5 rounded-xl border border-slate-100 text-xs">{{ $transaction->stage_1_notes }}</p>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
                    <h3 class="font-bold text-slate-800 border-b border-slate-100 pb-3 flex items-center justify-between">
                        <span class="flex items-center gap-2">
                            <i class="fa-solid fa-link text-indigo-600"></i> 2. ربط البوليصة والوكيل الملاحي
                        </span>
                    ...
                    <div class="grid grid-cols-2 gap-4 text-xs">
                        <div>
                            <span class="text-slate-400 block mb-1">الوكيل الملاحي (Shipping Agent):</span>
                        ...
                        <div>
                            ...
                            <span class="font-bold {{ $transaction->received_policy ? 'text-emerald-600' : 'text-amber-600' }}">
                                {{ $transaction->received_policy ? 'تم الاستلام' : 'لم تستلم بعد' }}
                            </span>
                        </div>
                        <div>
                            <span class="text-slate-400 block mb-1">تاريخ استلام البوليصة:</span>
                            <span class="font-bold text-slate-800">
                                {{ $transaction->policy_receipt_date ? (is_object($transaction->policy_receipt_date) ? $transaction->policy_receipt_date->format('Y-m-d') : $transaction->policy_receipt_date) : '---' }}
                            </span>
                        </div>
                        <div>
                            <span class="text-slate-400 block mb-1">فاتورة إذن التسليم:</span>
                            <span class="font-bold text-slate-800">{{ number_format($transaction->delivery_order_invoice ?? 0, 2) }} ر.س</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
                    <h3 class="font-bold text-slate-800 border-b border-slate-100 pb-3 flex items-center justify-between">
                        <span class="flex items-center gap-2">
                            <i class="fa-solid fa-building-columns text-amber-600"></i> 3. التخليص والبيان الجمركي
                        </span>
                    ...
                    <div class="grid grid-cols-2 gap-4 text-xs">
                        <div>
                            ...
                            <span class="font-bold text-slate-800 text-sm font-mono text-indigo-600">{{ $transaction->declaration_number ?? '---' }}</span>
                        </div>
                        <div>
                        ...
                        <div>
                            <span class="text-slate-400 block mb-1">أرقام الحاويات:</span>
                            <span class="font-bold text-slate-800 font-mono">{{ $transaction->container_numbers ?? '---' }}</span>
                        </div>
                        <div>
                            <span class="text-slate-400 block mb-1">فواتير الموانئ والمشغل:</span>
                            <span class="font-bold text-slate-800">{{ number_format(($transaction->ports_invoice ?? 0) + ($transaction->operator_invoice ?? 0), 2) }} ر.س</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
                    <h3 class="font-bold text-slate-800 border-b border-slate-100 pb-3 flex items-center justify-between">
                        <span class="flex items-center gap-2">
                            <i class="fa-solid fa-truck-fast text-purple-600"></i> 4 و 5. النقل وترجيع الفاضي
                        </span>
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4 md:col-span-2">
                    <h3 class="font-bold text-slate-800 border-b border-slate-100 pb-3 flex items-center justify-between">
                        <span class="flex items-center gap-2">
                            <i class="fa-solid fa-file-invoice-dollar text-emerald-600"></i> 6. الإنجاز والفاتورة والمبيعات والمرفقات
                        </span>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="md:col-span-2 border-t md:border-t-0 md:border-r border-slate-100 md:pr-6">
                            <span class="text-slate-500 text-xs block mb-2 font-bold flex items-center gap-1.5">
                                <i class="fa-solid fa-clock-rotate-left text-slate-400"></i> سجل حركة ومراحل هذا المستند:
                            </span>
