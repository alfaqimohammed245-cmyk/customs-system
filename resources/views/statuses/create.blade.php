<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h1 class="font-extrabold text-2xl text-slate-800 tracking-tight">إضافة حالة عملية جمركية جديدة</h1>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('statuses.store') }}" class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 space-y-6">
                @csrf
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-2">اسم الحالة *</label>
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="مثال: بانتظار الفحص المعملي..." class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none">
                </div>
                ...
                <div class="mt-8 pt-6 border-t border-slate-100 flex items-center justify-end gap-3">
                    ...
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-2.5 rounded-xl text-sm shadow-md transition">
                        <i class="fa-solid fa-plus ml-1"></i> حفظ الحالة
                    </button>
                </div>
            </form>
        </div>
